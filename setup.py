...
class cmake_build(build):
    ...
    def build_pymesh(self, cmake_args=""):
        """
        Config and build pymesh.
        """
        python_version = "{v[0]}.{v[1]}".format(v=platform.python_version_tuple())
        print(f"PyMesh: {cmake_args}")
        self._build(
            "build_{}".format(python_version),
            " {} -DPythonLibsNew_FIND_VERSION={}".format(cmake_args, python_version),
            False,
        )
    ...
    def run(self):
        cmake_args = f"-DGMP_LIBRARIES={os.environ['GMP_LIB']} -DGMP_INCLUDE_DIRS={os.environ['GMP_INC']} "
        cmake_args += f"-DMPFR_LIBRARIES={os.environ['MPFR_LIB']} -DMPFR_INCLUDE_DIR={os.environ['MPFR_INC']}"
        self.build_third_party()
        self.build_pymesh(cmake_args=cmake_args)
        build.run(self)
